using Dfe.ManageFreeSchoolProjects.API.RequestModels.Concerns.TeamCasework;
using Dfe.ManageFreeSchoolProjects.API.UseCases;
using Dfe.ManageFreeSchoolProjects.Data.Gateways;
using Dfe.ManageFreeSchoolProjects.Data.Models.Concerns.TeamCasework;
using FluentAssertions;
using Moq;
using System.Collections.Generic;
using System.Threading;
using System.Threading.Tasks;
using Xunit;

namespace Dfe.ManageFreeSchoolProjects.API.Tests.UseCases
{
    public class UpdateManageFreeSchoolProjectsTeamTests
    {
        [Fact]
        public async Task GetManageFreeSchoolProjectsTeam_Implements_IGetManageFreeSchoolProjectsTeam()
        {
            typeof(UpdateManageFreeSchoolProjectsTeam).Should().BeAssignableTo<IUpdateManageFreeSchoolProjectsTeam>();
        }

        [Fact]
        public async Task Execute_When_Team_Found_Performs_Update()
        {
            var ownerId = "john.doe";
            var mockGateway = new Mock<IConcernsTeamCaseworkGateway>();

            mockGateway
            .Setup(g => g.GetByOwnerId(ownerId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ManageFreeSchoolProjectsTeam
            {
                Id = ownerId,
                TeamMembers = new List<ManageFreeSchoolProjectsTeamMember>
                {
                    new ManageFreeSchoolProjectsTeamMember { TeamMember = "user.one" } ,
                    new ManageFreeSchoolProjectsTeamMember { TeamMember = "user.two" } ,
                    new ManageFreeSchoolProjectsTeamMember { TeamMember = "user.three" }
                }
            });

            var updateRequest = new ManageFreeSchoolProjectsTeamUpdateRequest()
            {
                OwnerId = ownerId,
                TeamMembers = new string[]
                {
                    "user.one",
                    "user.three"
                }
            };

            // Act
            var sut = new UpdateManageFreeSchoolProjectsTeam(mockGateway.Object);
            var result = await sut.Execute(updateRequest, CancellationToken.None);

            result.Should().NotBeNull();
            result.OwnerId.Should().Be(ownerId);
            result.TeamMembers.Length.Should().Be(2);
            result.TeamMembers.Should().Contain("user.one");
            result.TeamMembers.Should().Contain("user.three");

            mockGateway.Verify(x => x.UpdateCaseworkTeam(It.Is<ManageFreeSchoolProjectsTeam>(c => c.Id == ownerId && c.TeamMembers.Count == 2), It.IsAny<CancellationToken>()), Times.Once);
            mockGateway.Verify(x => x.AddCaseworkTeam(It.IsAny<ManageFreeSchoolProjectsTeam>(), It.IsAny<CancellationToken>()), Times.Never);
        }

        [Fact]
        public async Task Execute_When_Team_Not_Found_Performs_Add()
        {
            var ownerId = "john.doe";
            var mockGateway = new Mock<IConcernsTeamCaseworkGateway>();

            mockGateway
            .Setup(g => g.GetByOwnerId(ownerId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(default(ManageFreeSchoolProjectsTeam));

            var updateRequest = new ManageFreeSchoolProjectsTeamUpdateRequest()
            {
                OwnerId = ownerId,
                TeamMembers = new string[]
                {
                    "user.one",
                    "user.three"
                }
            };

            // Act
            var sut = new UpdateManageFreeSchoolProjectsTeam(mockGateway.Object);
            var result = await sut.Execute(updateRequest, CancellationToken.None);

            result.Should().NotBeNull();
            result.OwnerId.Should().Be(ownerId);
            result.TeamMembers.Length.Should().Be(2);
            result.TeamMembers.Should().Contain("user.one");
            result.TeamMembers.Should().Contain("user.three");

            // verify add was called.
            mockGateway.Verify(x => x.UpdateCaseworkTeam(It.IsAny<ManageFreeSchoolProjectsTeam>(), It.IsAny<CancellationToken>()), Times.Never);
            mockGateway.Verify(x => x.AddCaseworkTeam(It.Is<ManageFreeSchoolProjectsTeam>(c => c.Id == ownerId && c.TeamMembers.Count == 2), It.IsAny<CancellationToken>()), Times.Once);
        }
    }
}
