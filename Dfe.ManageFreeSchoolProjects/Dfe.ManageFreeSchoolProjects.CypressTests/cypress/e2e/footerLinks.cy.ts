...
describe("Navigate through footer links", () => {
    ...
    it('user should be able to navigate to the footer links', () => {
        ...
        cy.contains('a', 'Give feedback about a Regions Support system (opens in a new tab)').should('be.visible')
        cy.contains('a', 'Suggest a change about a Regions Support system (opens in a new tab)').should('be.visible')
